...
function getNextFriday(date) {
  ...
  dateObj.setDate(dateObj.getDate() + daysUntilFriday);
...
function getCountWeekendsInMonth(month, year) {
  ...
  const weekendCount = daysArray.reduce((count, day) => {
...
function getWeekNumberByDate(date) {
  ...
  const firstYear = new Date(currentDate.getFullYear(), 0, 1);
  const daysSinceStartOfYear =
    (currentDate - firstYear) / (1000 * 60 * 60 * 24);
  return Math.ceil((daysSinceStartOfYear + firstYear.getDay() + 1) / 7);
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const schedule = [];
  const startDate = new Date(reverse(period.start));
  const endDate = new Date(reverse(period.end));

  const currentDate = new Date(startDate);
  let dayCount = 0;

  while (currentDate.getTime() <= endDate.getTime()) {
    if (dayCount % (countWorkDays + countOffDays) < countWorkDays) {
